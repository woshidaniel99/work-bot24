...
function getMainKeyboard() {
  return {
    keyboard: [
      [{ text: "上班 / Bắt đầu làm" }, { text: "下班 / Tan làm" }],
      [{ text: "吃饭 / Ăn cơm" }, { text: "上厕所 / Vệ sinh" }, { text: "抽烟 / Hút thuốc" }],
      [{ text: "其他 / Khác" }],
      [{ text: "回座 / Trở lại chỗ" }],
    ],
...
// ─── STATUS LABELS ─────────────────────────────────────────────────────────
const STATUS_LABELS = {
  work:   "🟢 上班中 / Đang làm việc",
  eat:    "🍜 吃饭中 / Đang ăn cơm",
  toilet: "🚻 上厕所 / Đang vệ sinh",
  smoke:  "🚬 抽烟 / Đang hút thuốc",
  other:  "🔵 其他 / Khác",
  off:    "🔴 已下班 / Đã tan làm",
  idle:   "⬜ 未上班 / Chưa bắt đầu",
};
...
function startAwayTimer(userId, chatId, statusKey, mention, name) {
  ...
  session.awayTimer = setTimeout(() => {
    ...
    send(chatId,
      `⚠️ *超时提醒 / Cảnh báo quá giờ!*\n\n` +
      `${mention} 已经 ${STATUS_LABELS[statusKey]} *${awayLabel}*\n` +
      `已超过限制 ${limitLabel}，算迟到！请马上回座！\n` +
      `Đã quá giới hạn ${limitLabel} — bị tính trễ. Vui lòng trở lại chỗ ngay!`
    );
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(msg.chat.id,
    `👋 你好 ${msg.from.first_name || "朋友"}！\n\n请直接点击按钮打卡\nVui lòng nhấn nút để chấm công.\n\n状态 Trạng thái: ${STATUS_LABELS[session.status]}`,
...
bot.on("message", (msg) => {
  ...
  if (BLOCKED_PATTERNS.some(p => p.test(text))) {
    send(chatId, `🚫 *不允许自动打卡 / Không cho phép chấm công tự động!*\n\n${mention} 请手动点击按钮。\nVui lòng nhấn nút thủ công.`);
  ...
  if (text.includes("Bắt đầu") || text.includes("上班")) {
    ...
    let msg2 = `✅ *上班打卡成功 / Chấm công thành công!*\n👤 ${mention}\n⏰ 上班时间 Giờ vào: \`${camTime}\` ${COUNTRY}\n\n状态 Trạng thái: ${STATUS_LABELS["work"]}`;
    ...
    if (isLate()) {
      ...
      msg2 += `\n\n🚨 ${mention} *迟到 ${minsLate} 分钟 / TRỄ ${minsLate} phút!*\n⏰ 应在 ${startTimeStr} 上班 / Phải bắt đầu lúc ${startTimeStr}`;
  ...
  // ── OFF WORK ─────────────────────────────────────────────────────────────
  else if (text.includes("Tan làm") || text.includes("下班")) {
    if (session.status === "idle" || session.status === "off") {
    ...
    send(chatId,
      `🔴 *下班打卡 / Tan làm thành công!*\n👤 ${mention}\n⏰ 下班时间 Giờ ra: \`${camTime}\` ${COUNTRY}\n` +
      `🕐 总时间 Tổng: \`${formatDuration(totalMs)}\`\n💼 工作 Làm việc: \`${formatDuration(workMs)}\`\n🚶 离开 Rời chỗ: \`${formatDuration(session.totalAwayMs)}\``, true);
  ...
  // ── AWAY ACTIONS ─────────────────────────────────────────────────────────
    ...
    text.includes("Ăn cơm") || text.includes("吃饭") || text.includes("Vệ sinh") || text.includes("厕所") ||
    text.includes("Hút thuốc") || text.includes("抽烟") || text.includes("Khác") || text.includes("其他")
  ) {
    if (session.status === "idle" || session.status === "off") {
      return send(chatId, `⚠️ ${mention} 请先上班打卡！\nVui lòng chấm công trước.`);
    }
    ...
    if (text.includes("Ăn cơm")    || text.includes("吃饭")) { statusKey = "eat";    emoji = "🍜"; }
    if (text.includes("Vệ sinh") || text.includes("厕所")) { statusKey = "toilet"; emoji = "🚻"; }
    if (text.includes("Hút thuốc")  || text.includes("抽烟")) { statusKey = "smoke";  emoji = "🚬"; }
  ...
  // ── BACK TO SEAT ─────────────────────────────────────────────────────────
    ...
    let msg2 =
      `💺 ${mention} *回座成功 / Trở lại chỗ!*\n` +
      `🔙 来自 Đến từ: ${fromBreak}\n` +
      `🕐 回座时间 Giờ: \`${camTime}\` ${COUNTRY}\n` +
      `⏱ 本次离开 Lần này rời: \`${formatDuration(awayDuration)}\`\n` +
      `🚶 今日总离开 Tổng rời hôm nay: \`${formatDuration(session.totalAwayMs)}\`\n` +
      `⏰ 上班时间 Giờ vào: \`${session.clockInTime || "—"}\`\n` +
      `💼 已工作 Đã làm được: \`${formatDuration(currentWorkMs)}\``;

    if (wasOvertime) {
      msg2 += `\n\n⚠️ *超时算迟到 / Quá giờ: ${formatDuration(awayDuration - dispLimitMs)}!*`;
